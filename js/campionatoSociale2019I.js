...
//Anche se non esiste lo devo lasciare altrimenti gli indici non corrispondono e i gironi non avviati risultano sbagliati.       
        stgironi += ',{"index": "51", "nome": "csp-inverno-2018-2019-girone-51", "descrizione" : "51", "partiteTerminate" : 0, "partiteGirone" : 30, "avviato" : 2, "daCaricare" : true,  "risultati" : "{}"}';
...
stgironi += ']}';
    ...
    CAMPIONATO.gironi.girone[14-1].dataFine = "07/05/2019";   
    CAMPIONATO.gironi.girone[14-1].partiteTerminate = 42;   
...
username = 'napoli67';CAMPIONATO.giocatori[username] = {}; stgiocatore = '{"username":"napoli67","displayName":"napoli67", "id":"https://www.chess.com/member/napoli67","avatar":"","elo":"", "punteggio":8.91, "vinte":5, "perse":4, "patte":3, "partiteTotali":12, "partiteTerminate":12,"gironi":"", "stampato":false, "accountChiuso":false}';CAMPIONATO.giocatori[username] = JSON.parse(stgiocatore);
...
username = 'silverblack';CAMPIONATO.giocatori[username] = {}; stgiocatore = '{"username":"silverblack","displayName":"silverblack", "id":"https://www.chess.com/member/silverblack","avatar":"","elo":"", "punteggio":8.4, "vinte":6, "perse":5, "patte":1, "partiteTotali":12, "partiteTerminate":12,"gironi":"", "stampato":false, "accountChiuso":false}';CAMPIONATO.giocatori[username] = JSON.parse(stgiocatore);
username = 'linchetto';CAMPIONATO.giocatori[username] = {}; stgiocatore = '{"username":"linchetto","displayName":"linchetto", "id":"https://www.chess.com/member/linchetto","avatar":"","elo":"", "punteggio":5.47, "vinte":4, "perse":7, "patte":1, "partiteTotali":12, "partiteTerminate":12,"gironi":"", "stampato":false, "accountChiuso":false}';CAMPIONATO.giocatori[username] = JSON.parse(stgiocatore);
...
console.log('------------------------------------------ inizio elaborazione: ' + oraElaborazione + ' - ' + oraElaborazione.getTime());  //??????????????????
...
console.log('-------------- caricaDati lancio calcola classifica --------'); //???????????                
               ...
               // CAMPIONATO.caricaDati(this.url);    
                ...
                if (! jqXhr.responseJSON)
                {
                    ...
                    console.log('-------------- caricaDati ERRORE lancio calcola classifica --------'); //???????????                
                    CAMPIONATO.calcolaClassifica();
                }
    ...
    calcolaClassifica: function()
    {
        ...
        console.log('------------------------------------------ inizio calcolaClassifica: ' + millisec);  //??????????????????
        var eloWhite = 0;
        var eloBlack = 0;
        var png = '';
        //Per tutti i gironi
        for (var i in CAMPIONATO.gironi.girone) {
           ...
           //Aggiorno partite totali del girone
            ...
            for (var iGames in CAMPIONATO.gironi.girone[i].risultati.games) {
                ...
                if ((i==63) &&(CAMPIONATO.gironi.girone[i].risultati.games[iGames].white == 'SouthItaly'))
                {
                    var a = CAMPIONATO.gironi.girone[i].risultati.games[iGames].black.username;
                }
        ...
        oraAttuale = new Date();
        var millisec = oraElaborazione.getTime() - oraAttuale.getTime();
        console.log('------------------------------------------ Fine calcolaClassifica: '  + millisec);  //??????????????????
    ...
    getAvatar : function()
    {
        ...
        oraAttuale = new Date();
        var millisec = oraElaborazione.getTime() - oraAttuale.getTime();
        console.log('------------------------------------------ inizio getAvatar: '  + millisec);  //??????????????????
        //Cerco il primo giocatore di cui non so ancora l'avatar
        //  Potrebbe essere andata in erore l'api di ricerca profilo
        for (var username in CAMPIONATO.giocatori) {
                //Cerco avatar
                if (CAMPIONATO.defaultAvatar[username]) {
                    //console.log('Assegno avatar di default: '  + username);
                    CAMPIONATO.giocatori[username].avatar = CAMPIONATO.defaultAvatar[username];
                } else {
    ...
    getAvatarUrl: function(url)
    {
        ...
        console.log('Inizio getAvatarUrl: '  + url);
    ...
    setPunti : function(risultato, elo, mosseOk, end_time, avversarioUsename, avversarioElo, avversarioStato, index) {
        ...
        //oraAttuale = new Date();
        //var millisec = oraElaborazione.getTime() - oraAttuale.getTime();
        //console.log('------------------------------------------ Inizio  setPunti: ' +  millisec);  //??????????????????
        //??????????????
        ...
        var mioGirone = parseInt(index) + 1;
        var punti;
        if (risultato.result == 'win')
        {
            ...
            CAMPIONATO.giocatori[risultato.username.toLowerCase()].calcoloPunti += ' - W - ' +punti + ' (#' + (mioGirone) + ')';
        } else {
            if ((risultato.result == 'agreed') || (risultato.result == 'repetition')  || (risultato.result == 'timevsinsufficient') || 
                ...
                CAMPIONATO.giocatori[risultato.username.toLowerCase()].calcoloPunti += '&nbsp;&nbsp;' + risultato.username + ' (' + elo + ')';
                CAMPIONATO.giocatori[risultato.username.toLowerCase()].calcoloPunti += ' - ' + avversarioUsename +  ' (' + avversarioElo + ')';
                CAMPIONATO.giocatori[risultato.username.toLowerCase()].calcoloPunti += ' - D - ' + punti + ' (#' + (mioGirone) + ')';
            } else {
                //Perso      
    ...
    getPunti: function(elo, avversarioElo)
    {
        ...
        //oraAttuale = new Date();
        //var millisec = oraElaborazione.getTime() - oraAttuale.getTime();
        //console.log('------------------------------------------ Inizio  getPunti: ' + millisec);  //??????????????????
        //??????????????
    ...
    controllaInPartenza: function()
    {
        ...
        oraAttuale = new Date();
        var millisec = oraElaborazione.getTime() - oraAttuale.getTime();
        console.log('------------------------------------------ Inizio  controllaInPartenza: ' +  millisec);  //??????????????????
        //??????????????
        ...
        for (var i in CAMPIONATO.gironi.girone) {
            ...
            if (CAMPIONATO.gironi.girone[i].avviato != 1)
                ...
                console.log('------------------------------------------ Inizio  controllo girone: ' +CAMPIONATO.gironi.girone[i].nome+ ' '+  millisec);  //??????????????????
                $.getJSON('https://api.chess.com/pub/tournament/' + CAMPIONATO.gironi.girone[i].nome,function(data){
            
                    console.log('------------------------------------------ Inizio  controllo dati girone: ' +CAMPIONATO.gironi.girone[i].nome+ ' '+  millisec);  //??????????????????
                ...
                //leggo il girone da aggiornare
                var mioGirone = this.url.substr(37, this.url.length-37);
                console.log('------------------------------------------ Inizio  controllo ERRORE: ' + mioGirone + '  '  +  millisec);  //??????????????????
                var iGirone = 0
                for (var ii in CAMPIONATO.gironi.girone) {
                    if (mioGirone == CAMPIONATO.gironi.girone[ii].nome) {
                        iGirone = ii
        ...
        console.log('------------------------------------------ Fine  controlloInPartenza: ');  //??????????????????
    ...
    scriviTabelle: function()
    {
        ...
        oraAttuale = new Date();
        var millisec = oraElaborazione.getTime() - oraAttuale.getTime();
        console.log('------------------------------------------ Inizio  scriviTabelle: ' + millisec);  //??????????????????
        //??????????????
        ...
        oraAttuale = new Date();
        var millisec = oraElaborazione.getTime() - oraAttuale.getTime();
        console.log('------------------------------------------ Inizio  calcola classifica: ' + millisec);  //??????????????????
        //??????????????
        //Tabella classifica
        posizione.N = 0;
        posizione.oldPunti = -1;
